...
// @author       Cryosis [2030298?] (rehost by Helcostr [1934501])
// @match        https://www.torn.com/factions.php?step=profile&*ID=*
// @grant        GM_addStyle
// ==/UserScript==
/**
 * Because the original author seems to have gone dark, I have taken it upon myself to rehost and correct the script. 
 */
...
$(window).load(function () {
  ...
  function filterMember(index) {
      ...
      $(checkboxes).each(function(index) {
          if ($(this).prop('checked')) {
              switch (this.name) {
                  case 'activity':
                  console.log((filters['activity'] === 'active') + '&&' + (player['activity'] === 'offline'));
  ...
  function createPlayer(rawData) {
      ...
      console.log(playerActivityIconID);
      console.log(/icon\d+/.exec(playerActivityIconID)[0]);
      let playerActivity;
      switch(/icon\d+/.exec(playerActivityIconID)[0]) {
          case 'icon1': playerActivity = 'online';
          break;
          case 'icon2': playerActivity = 'offline';
          break;
          case 'icon62': playerActivity = 'idle';
          break;
          default: playerActivity = 'help im broken UwU';
      }
