...
// @description  DO NOT RELEASE Parse the Torn Territory War Report
...
(function() {
    ...
    const createJSON = ()=>{
        ...
        if (resText.search("failed") != -1)
            ...
            result = 0;
        let report = {
            author:parseInt(/XID=(\d+)/.exec($("[class^=menu-name]").next().attr("href"))[1]),
            ...
            ts_start:Date.UTC(ts[3], ts[1], ts[2], ts[4], ts[5], ts[6]) / 1000,
            ts_end:Date.UTC(ts[3+6], ts[1+6], ts[2+6], ts[4+6], ts[5+6], ts[6+6]) / 1000,
